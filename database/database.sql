...
(
    ...
    `create_time`        INTEGER DEFAULT 0,  -- 创建时间戳，单位s
    `update_time`        INTEGER DEFAULT 0   -- 修改时间戳，单位s
);

INSERT INTO `user`(`name`, `password`, `create_time`) VALUES ('admin', '$2a$10$ZsS2bA7B7AQtIBBpW7xz3OIw3FWU0CnXX7HZMi6vBNt9ZNcA2RNGG', 1675393080);
...
(
    ...
    `type`        TEXT    DEFAULT '',                -- 类型，folder、md、pdf、zip
    `size`        INTEGER DEFAULT 0,                 -- 大小，单位byte
    `delete`      INTEGER DEFAULT 0,                 -- 是否已删除，0-否，1-是
    `create_time` INTEGER DEFAULT 0,                 -- 创建时间戳，单位s
    `update_time` INTEGER DEFAULT 0                  -- 修改时间戳，单位s
);
