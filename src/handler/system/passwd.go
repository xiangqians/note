...
// Passwd 密码
func Passwd(request *http.Request, session *session.Session) (string, model.Response) {
	// 请求方法
	method := request.Method

	// 密码页
	if method != http.MethodPost {
		return "system/passwd", model.Response{
			Msg: session.GetMsg(),
		}
	}
	...
	errRedirect := func(err any) (string, model.Response) {
		return "redirect:/passwd", model.Response{
